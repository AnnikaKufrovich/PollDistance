...
```{r}
ALApollplace <- data.frame(precinct = prec_numbers, address = poll_address, pollname = poll_names, abr = c(rep("ALA", length(prec_numbers))))
head(ALApollplace)
...
FLpollplace <- data.frame(abr = countyabr, precinct = prec_nums, address = addresses, pollname = pollnames)
head(FLpollplace)
