...
for(i in 1:length(endings)){
  webpage <- read_html(paste0(incomplete_url, endings[i])) ##county specific webpage
  ...
  prec.numbers <- str_remove_all(number.data, "Precinct ")
  length <- length(prec.numbers) #needed to add county abbreviation for each precinct entry
  countyabr <- c(countyabr, rep(endings[i], length)) #Adding county abreviation for each entry, can be changed later to appropriate abbreviations
  prec_nums <- c(prec_nums, prec.numbers)
}
